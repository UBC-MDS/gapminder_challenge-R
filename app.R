        ...
        list(
          ...
          dbcCol(dbcCard(className = "m-3 p-3", div(
            list(
              ...
              dccRangeSlider(
                min = 1918, max = 2018, step = 1 - 10, value = list(1918, 2018), id = "year_range_slider",
                marks = marks_card2
              ),
...
app$callback(
  ...
  function(selected_year, selected_regions) {
    p <- ggplot(df_year_region %>%
      ...
      aes(
        y = reorder(region, sum_co2_per_capita, ),
...
app$callback(
  ...
  function(selected_year, selected_regions) {
    ...
    p <- ggplot(df_subregion_year %>%
      filter(
        year >= selected_year[1], year <= selected_year[2],
